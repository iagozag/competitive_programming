...
namespace seg {
	...
	tp update(int a, int x, int p=1, int l=0, int r=n-1) {
		if (a < l or r < a) return seg[p];
		if (l == r) return seg[p] = {x, x, x, x};
		int m = (l+r)/2;
		return seg[p] = junta(update(a, x, 2*p, l, m), update(a, x, 2*p+1, m+1, r));
	}
...
void solve(){
	...
	for(int i = 0; i < q; i++){
		int a, b; cin >> a >> b; --a;
		seg::update(a, b);
		cout << max(0LL, get<3>(seg::query(0, n-1))) << endl;
	}
}
