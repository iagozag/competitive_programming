...
namespace beats {
	struct node {
		...
		void setmin(ll x, bool compute) {
			if (x >= ma1) return;

			if(compute) seg::update(x+1, ma1, -ma);
	...
	void prop(int p, int l, int r) {
		if (l == r) return;
		for (int k = 0; k < 2; k++)
			seg[2*p+k].setmin(seg[p].ma1, false);
	...
	node update(int a, ll x, int p=1, int l=0, int r=n-1) {
		if (l == r) {
			int prev = seg[p].ma1;
            seg::update(a+1, prev, -1);
            seg::update(a+1, x, 1);
		...
		prop(p, l, r);
		int m = (l+r)/2;
		if(a <= m) seg[p] = {update(a, x, 2*p, l, m), seg[p*2+1]};
		else seg[p] = {seg[2*p], update(a, x, 2*p+1, m+1, r)};
...
void solve(){
	int n, q; cin >> n >> q;
	int w[n], ans[n];
	for(int i = 0; i < n; i++) w[i] = i, ans[i] = 1;
