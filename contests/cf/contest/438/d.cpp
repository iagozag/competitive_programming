...
namespace beats {
	...
	node updatemod(int a, int b, ll x, int p=1, int l=0, int r=n-1) {
		if (b < l or r < a or seg[p].ma1 < x) return seg[p];
		if (a <= l and r <= b and seg[p].mi1 == seg[p].ma1) {
			seg[p].setmin(seg[p].mi1%x);
			return seg[p];
		}
		prop(p, l, r);
		int m = (l+r)/2;
		return seg[p] = {updatemod(a, b, x, 2*p, l, m),
...
void solve(){
	...
	for(int i = 0; i < q; i++){
		...
		if(t == 1){
			int l, r; cin >> l >> r; --l, --r;
			cout << beats::query(l, r).second << endl;
		} else if(t == 2){
			int l, r, x; cin >> l >> r >> x; --l, --r;
			beats::updatemod(l, r, x);
		} else{
			int k, x; cin >> k >> x; --k;
			beats::updatemin(k, k, x), beats::updatemax(k, k, x);
		}
	}
