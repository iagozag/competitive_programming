...
struct hopcroft_karp{
	...
	hopcroft_karp(int n_, int m_): n(n_), m(m_), g(n), dist(n), nxt(n), ma(n, -1), mb(m, -1) {}
	...
	int matching(){
		...
		while(bfs()){
			...
			for(int i = 0; i < n; i++) if(ma[i] == -1 and dfs(i)) ret++;
		}
...
int32_t main(){
	...
	for(int i = 0; i < n; i++){
		string s; cin >> s;
		for(int j = 0; j < n; j++){
			if(i < j and s[j] == '0') h.add(i, j);
		}
	}

	cout << h.matching()+1 << endl;
	exit(0);
}
